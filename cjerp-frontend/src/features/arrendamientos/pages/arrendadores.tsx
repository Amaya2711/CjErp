...
const initialForm = (): ArrendadorForm => ({
  id: null,
  codigo: generarCodigoArrendador(),
  nombre: "",
  detalle: "",
  estado: "ACTIVO",
  observacion: "",
...
export default function ArrendamientosArrendadoresPage() {
  return (
    <ArrendamientosCrudPage<ArrendadorForm>
      title="Arrendadores"
      ...
      mapRowToForm={(row) => ({
        id: row.id ?? null,
        codigo: row.codigo ?? generarCodigoArrendador(),
        nombre: row.nombre ?? "",
        detalle: row.detalle ?? "",
      ...
      saveForm={async (payload, mode) => {
        const result = await crearArrendadorArrendamientos(payload);
        return {
          message:
            result.message ||
            (mode === "nuevo" ? "Arrendador creado correctamente." : "Arrendador actualizado correctamente."),
        };
      }}
      validateForm={(form) => {
        const errors: Record<string, string> = {};
        if (!form.nombre.trim()) errors.nombre = "Ingrese la razon social.";
      ...
      renderForm={(form, setForm, errors, mode) => (
        <div style={styles.grid}>
          ...
          <Field label="Razon social" error={errors.nombre}>
            <input
              value={form.nombre}
              onChange={(event) => setForm((prev) => ({ ...prev, nombre: event.target.value }))}
              style={getFieldStyle(Boolean(errors.nombre))}
            />
          </Field>
          <Field label="Detalle">
            <input
...
function generarCodigoArrendador() {
  const now = new Date();
