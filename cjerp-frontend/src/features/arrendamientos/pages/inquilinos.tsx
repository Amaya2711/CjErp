...
const initialForm = (): InquilinoForm => ({
  id: null,
  codigo: generarCodigoInquilino(),
  nombre: "",
  detalle: "",
  estado: "ACTIVO",
  observacion: "",
...
export default function ArrendamientosInquilinosPage() {
  return (
    <ArrendamientosCrudPage<InquilinoForm>
      title="Inquilinos"
      ...
      mapRowToForm={(row) => ({
        id: row.id ?? null,
        codigo: row.codigo ?? generarCodigoInquilino(),
        nombre: row.nombre ?? "",
        detalle: row.detalle ?? "",
      ...
      saveForm={async (payload, mode) => {
        const result = await crearInquilinoArrendamientos(payload);
        return {
          message:
            result.message ||
            (mode === "nuevo" ? "Inquilino creado correctamente." : "Inquilino actualizado correctamente."),
        };
      }}
      validateForm={(form) => {
        const errors: Record<string, string> = {};
        if (!form.codigo.trim()) errors.codigo = "Ingrese el codigo.";
