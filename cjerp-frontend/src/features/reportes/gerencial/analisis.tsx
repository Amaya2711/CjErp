...
export default function AnalisisPage() {
  ...
  const scopeSelection = useMemo(() => {
    ...
    if (appliedLookupFiltro) {
      ...
      if (exactRows.length > 0) {
        return {
          rows: exactRows,
          level: (lookupSite ? "site" : lookupProyecto ? "proyecto" : "cliente") as LevelKey,
        };
      }
      ...
      if (projectRows.length > 0) {
        return {
          rows: projectRows,
          level: lookupProyecto ? ("proyecto" as LevelKey) : ("cliente" as LevelKey),
        };
      }

      const clientRows = dateFilteredMovements.filter((row) => matches(row.cliente, lookupCliente));
      if (clientRows.length > 0) {
        return { rows: clientRows, level: "cliente" as LevelKey };
      }

      return { rows: dateFilteredMovements, level: "cliente" as LevelKey };
    }

    const selectedRows = dateFilteredMovements.filter((row) => {
      if (selectedCliente && row.cliente !== selectedCliente) return false;
    ...
    return {
      rows: selectedRows,
      level: !selectedCliente ? ("cliente" as LevelKey) : !selectedProyecto ? ("proyecto" as LevelKey) : ("site" as LevelKey),
    };
  ...
  const resolvedClienteForHierarchy = selectedCliente || appliedLookupFiltro?.nombreCliente || "";
  ...
  const displayRanking = useMemo(() => {
    if (activeLevel !== "cliente") {
      ...
      const sourceProjects =
        masterProyectos.length > 0
          ? masterProyectos
              .filter((row) => normalizeText(row.nombreCliente) === normalizeText(resolvedClienteForHierarchy))
              .map((row) => row.nombreProyecto)
          : [];
    ...
    for (const cliente of masterClientes) {
    ...
    return Array.from(byLabel.values()).sort((left, right) => left.netoPen - right.netoPen || left.count - right.count || left.label.localeCompare(right.label, "es", { sensitivity: "base" }));
  }, [activeLevel, masterClientes, masterProyectos, resolvedClienteForHierarchy, selectedRanking]);
  ...
  const proyectoOptions = useMemo(() => {
    if (!resolvedClienteForHierarchy) return [];
    ...
    const source =
      masterProyectos.length > 0
        ? masterProyectos
            ...
            .map((row) => row.nombreProyecto)
        : movements
            .filter((row) => row.cliente === resolvedClienteForHierarchy)
            .map((row) => row.proyecto);

    return Array.from(new Set(source.filter(Boolean))).sort((left, right) =>
      left.localeCompare(right, "es", { sensitivity: "base" }),
    );
  }, [masterProyectos, movements, resolvedClienteForHierarchy]);
  const siteOptions = useMemo(() => {
    if (!resolvedClienteForHierarchy || !resolvedProyectoForHierarchy) return [];
    return Array.from(
      new Set(
        movements
          .filter((row) => row.cliente === resolvedClienteForHierarchy && row.proyecto === resolvedProyectoForHierarchy)
          .map((row) => row.site)
          .filter(Boolean),
      ),
    ).sort((left, right) => left.localeCompare(right, "es", { sensitivity: "base" }));
  }, [movements, resolvedClienteForHierarchy, resolvedProyectoForHierarchy]);
          ...
          <div style={styles.analysisGrid}>
            <AppCard style={styles.analysisChartCard}>
              <AppSectionHeader
                ...
                description={
                  !selectedCliente
                    ? "Primero se muestran clientes."
                    : !selectedProyecto
                      ? "Selecciona un cliente para ver proyectos."
                      : !selectedSite
              ...
              <div style={styles.chartBreadcrumb}>
                ...
                <div style={styles.chartBreadcrumbTrail}>
                  ...
                  {selectedCliente ? <ChevronRight size={14} strokeWidth={2.2} color="#94A3B8" /> : null}
                  {selectedCliente ? (
                    <button
                      type="button"
                  ...
                  {selectedProyecto ? <ChevronRight size={14} strokeWidth={2.2} color="#94A3B8" /> : null}
                  {selectedProyecto ? (
                    <button
                      type="button"
