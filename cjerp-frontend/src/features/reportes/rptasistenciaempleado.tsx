...
export default function RptAsistenciaPage() {
  ...
  const chartEmpleadoPorDia = useMemo(() => {
    ...
    filteredRows.forEach((item) => {
      ...
      if (estadoMarcacion) {
        ...
        const employeeStateCountMap = employeeStateCounts.get(employee)!;
        employeeStateCountMap.set(estadoMarcacion, (employeeStateCountMap.get(estadoMarcacion) ?? 0) + 1);
      }
    ...
    const rows = employees.map((employee) => {
      ...
      return {
        ...
        stateCounts: Object.fromEntries((employeeStateCounts.get(employee) ?? new Map<string, number>()).entries()),
      };
    });
  ...
  const isExcelExportDisabled = useMemo(() => {
    if (activeTab === "cuadros") {
      return filteredEmployeeGridRows.length === 0 || employeeStateGridStates.length === 0;
    }
  ...
  const exportPdf = async () => {
    if (activeTab === "cuadros") {
      ...
      doc.setFontSize(16);
      doc.text("Cuadro por estado", 14, 16);
      doc.setFontSize(10);
      ...
      doc.save(`cuadro_${toApiDate(fechaInicio).replaceAll("/", "")}_${toApiDate(fechaFin).replaceAll("/", "")}.pdf`);
      return;
    }
      ...
      const pdfItems: AsistenciaReportePdfItem[] = filteredRows
        ...
        .map((item) => {
          ...
          return {
            ...
            correoEmpleado: item.correoEmpleado ?? "",
        ...
        <section style={{ ...styles.tableCard, ...styles.employeeGridSection }}>
          <div style={styles.filterHeader}>
            <div>
              ...
              <p style={styles.sectionText}>Muestra el resumen por empleado con columnas por estado de marcacion.</p>
            </div>
            <div style={styles.counterPill}>{filteredEmployeeGridRows.length} filas</div>
          </div>
          <ChartCard title="" subtitle="" style={styles.employeeGridChartCard}>
        ...
        >
          <div style={{ ...styles.stateDateGridHeader, ...styles.stateDateGridCorner }}>
            <div style={styles.employeeGridHeaderStack}>
              <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("employee")}>
                <span>Empleado</span>
          ...
          <div style={styles.stateDateGridHeader}>Telefono</div>
          <div style={styles.stateDateGridHeader}>
            <div style={styles.employeeGridHeaderStack}>
              <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("responsable")}>
          ...
          <div style={styles.stateDateGridHeader}>
            <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("total")}>
          ...
          <div style={styles.stateDateGridHeader}>
            <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("otros")}>
              <span>Hrs Otros</span>
              <span style={styles.employeeGridSortPill}>
          ...
          <div style={styles.stateDateGridHeader}>
            <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("faltaAprobar")}>
          ...
          <div style={styles.stateDateGridHeader}>
            <div style={styles.employeeGridHeaderStack}>
              <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort("diferencia")}>
          ...
          <div style={styles.stateDateGridHeader}>
            <div style={styles.employeeGridHeaderStack}>
              ...
              <select value={validationFilter} onChange={(event) => onValidationFilterChange(event.target.value)} style={styles.employeeGridHeaderSelect}>
          ...
          {states.map((state) => {
            ...
            return (
              <div key={`head-state-${state}`} style={styles.stateDateGridHeader}>
                <div style={styles.employeeGridHeaderStack}>
                  <button type="button" style={styles.employeeGridSortButton} onClick={() => onToggleSort(stateSortKey)}>
                    <span>{state}</span>
                    <span style={styles.employeeGridSortPill}>
          ...
          {data.length === 0 ? (
            <div style={styles.employeeGridEmptyRow}>No hay filas que coincidan con los filtros actuales.</div>
          ) : data.map((item) => {
