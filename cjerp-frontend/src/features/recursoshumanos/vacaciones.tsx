...
const GASTOS_HEADER_FILTERS_INITIAL: GastosHeaderFilters = {
  ...
  idOc: "",
  porcentajeFic: "",
  fechaInicio: obtenerFechaInicioMesActual(),
...
function obtenerFechaInicioMesActual(): string {
...
function mapPlanillaConsultaRowToGastoDto(row: Record<string, unknown>, index: number): GastoDto {
  return {
    id: getRecordNumber(row, "IdEmpleadoCj", "idEmpleadoCj", "Id", "id") ?? index + 1,
    ...
    fechaFin: getRecordString(row, "FechaFin", "fechaFin"),
    usuario: getRecordString(row, "Usuario", "usuario"),
    fechaCreacion: getRecordString(row, "FechaCreacion", "fechaCreacion"),
    idResponsableCj: getRecordNumber(row, "IdResponsableCj", "idResponsableCj") ?? undefined,
    idSegundoVacaciones: getRecordNumber(row, "IdSegundoVacaciones", "idSegundoVacaciones") ?? undefined,
    idTerceroVacaciones: getRecordNumber(row, "IdTerceroVacaciones", "idTerceroVacaciones") ?? undefined,
    primerValidador: getRecordString(row, "PrimerValidador", "primerValidador"),
    segundoValidador: getRecordString(row, "SegundoValidador", "segundoValidador"),
    tercerValidador: getRecordString(row, "TercerValidador", "tercerValidador"),
    responsableVacaciones: getRecordString(row, "Responsable", "responsable"),
    fechaAprob: getRecordString(row, "FechaAprob", "fechaAprob"),
    saldoVacaciones: getRecordNumber(row, "SaldoVacaciones", "saldoVacaciones") ?? getRecordString(row, "SaldoVacaciones", "saldoVacaciones"),
    idOc: undefined,
    idSuministroProvisional: undefined,
    fechaInicioSuministroProvisional: "",
    filtroOperativoKey: "",
    responsable: getRecordString(row, "Responsable", "responsable"),
    responsableLabel: getRecordString(row, "Responsable", "responsable"),
    idBancoCta: undefined,
    ...
    tipoCambio: TIPO_CAMBIO_GASTO,
    subOc: undefined,
    subPlanilla: undefined,
    porce: undefined,
    idMonedaOc: undefined,
    adelaFic: undefined,
    diferenciaFic: undefined,
    codigoValidacionFic: undefined,
    resultadoValidacionFic: "",
    porcentajeFic: undefined,
  };
}
...
function mapGastoDtoToView(item: GastoDto): GastoForm {
  ...
  return {
    ...
    primerValidador: item.primerValidador,
    segundoValidador: item.segundoValidador,
    tercerValidador: item.tercerValidador,
    responsableVacaciones: item.responsableVacaciones,
    fechaInicio: item.fechaInicio,
    fechaFin: item.fechaFin,
    usuario: item.usuario,
    fechaCreacion: item.fechaCreacion,
    fechaAprob: item.fechaAprob,
    saldoVacaciones: item.saldoVacaciones,
    };
}
...
const vacacionFormularioInicial = (fechaActual: string): VacacionNuevoForm => ({
  idEmpleadoCj: "",
...
export default function RecursosHumanosVacacionesPage() {
  ...
  const [vacacionPanelAbierto, setVacacionPanelAbierto] = useState(false);
  const [vacacionGuardando, setVacacionGuardando] = useState(false);
  ...
  const cerrarVacacionPanel = () => {
    setVacacionPanelAbierto(false);
    setVacacionError(null);
    setVacacionErrores({});
    setVacacionForm(vacacionFormularioInicial(fechaActual));
    setEmpleadoVacacionInput("");
  ...
  const actualizarVacacionCampo = <K extends keyof VacacionNuevoForm>(campo: K, valor: VacacionNuevoForm[K]) => {
    setVacacionForm((prev) => ({ ...prev, [campo]: valor }));
    setVacacionErrores((prev) => {
      if (!prev[campo]) {
        return prev;
      }
  ...
  const guardarVacacionNueva = async () => {
    const erroresVacacion: Record<string, string> = {};

    if (!vacacionForm.idEmpleadoCj) erroresVacacion.idEmpleadoCj = "Seleccione el empleado.";
    if (!vacacionForm.fechaInicio) erroresVacacion.fechaInicio = "Seleccione la fecha de inicio.";
    if (!vacacionForm.fechaFin) erroresVacacion.fechaFin = "Seleccione la fecha fin.";
  ...
  const camposBusquedaGastos = useMemo<CrudToolbarSearchField<GastoForm>[]>(
    () => [
      ...
      { key: "idEmpleadoCj", label: "Id empleado", getValue: (gasto) => gasto.idEmpleadoCj ?? "" },
      { key: "nombreEmpleado", label: "Empleado", getValue: (gasto) => gasto.nombreEmpleado ?? "" },
      { key: "idEstado", label: "Id estado", getValue: (gasto) => gasto.estado ?? "" },
      { key: "estado", label: "Estado", getValue: (gasto) => gasto.estadoLabel ?? "" },
      { key: "idActivo", label: "Activo", getValue: (gasto) => gasto.idActivo ?? "" },
      { key: "fechaInicio", label: "Fecha inicio", getValue: (gasto) => gasto.fechaInicio ?? "" },
      { key: "fechaFin", label: "Fecha fin", getValue: (gasto) => gasto.fechaFin ?? "" },
      { key: "usuario", label: "Usuario", getValue: (gasto) => gasto.usuario ?? "" },
      { key: "fechaCreacion", label: "Fecha creación", getValue: (gasto) => gasto.fechaCreacion ?? "" },
      { key: "idResponsableCj", label: "Id responsable", getValue: (gasto) => gasto.idResponsableCj ?? "" },
      { key: "idSegundoVacaciones", label: "Id segundo", getValue: (gasto) => gasto.idSegundoVacaciones ?? "" },
      { key: "idTerceroVacaciones", label: "Id tercero", getValue: (gasto) => gasto.idTerceroVacaciones ?? "" },
      { key: "primerValidador", label: "Primer validador", getValue: (gasto) => gasto.primerValidador ?? "" },
      { key: "segundoValidador", label: "Segundo validador", getValue: (gasto) => gasto.segundoValidador ?? "" },
      { key: "tercerValidador", label: "Tercer validador", getValue: (gasto) => gasto.tercerValidador ?? "" },
      { key: "responsableVacaciones", label: "Responsable", getValue: (gasto) => gasto.responsableVacaciones ?? gasto.responsableLabel ?? gasto.responsable ?? "" },
      { key: "fechaAprob", label: "Fecha aprobación", getValue: (gasto) => gasto.fechaAprob ?? "" },
      { key: "saldoVacaciones", label: "Saldo vacaciones", getValue: (gasto) => gasto.saldoVacaciones ?? "" },
    ],
    []
  );
    ...
    (gasto: GastoForm, key: string): string | number => {
      switch (key) {
        case "id":
          return gasto.id ?? "";
        case "idEmpleadoCj":
          return gasto.idEmpleadoCj ?? "";
        case "nombreEmpleado":
          return gasto.nombreEmpleado ?? "";
        case "idEstado":
          return gasto.estado ?? "";
        case "estado":
          return gasto.estadoLabel ?? "";
        case "idActivo":
          return gasto.idActivo ?? "";
        case "fechaInicio":
          return formatInputDateForDisplay(gasto.fechaInicio);
        case "fechaFin":
          return formatInputDateForDisplay(gasto.fechaFin);
        case "usuario":
          return gasto.usuario ?? "";
        case "fechaCreacion":
          return formatInputDateForDisplay(gasto.fechaCreacion);
        case "idResponsableCj":
          return gasto.idResponsableCj ?? "";
        case "idSegundoVacaciones":
          return gasto.idSegundoVacaciones ?? "";
        case "idTerceroVacaciones":
          return gasto.idTerceroVacaciones ?? "";
        case "primerValidador":
          return gasto.primerValidador ?? "";
        case "segundoValidador":
          return gasto.segundoValidador ?? "";
        case "tercerValidador":
          return gasto.tercerValidador ?? "";
        case "responsableVacaciones":
          return gasto.responsableVacaciones ?? gasto.responsableLabel ?? gasto.responsable ?? "";
        case "fechaAprob":
          return formatInputDateForDisplay(gasto.fechaAprob);
        case "saldoVacaciones":
          return gasto.saldoVacaciones != null ? String(gasto.saldoVacaciones) : "";
        default:
  ...
  const columnasGridGastos: GridColumnDef[] = [
    { key: "seleccion", label: "", width: "44px", align: "center" as const, fixed: "left" },
    { key: "idEmpleadoCj", label: "Id", width: "80px", align: "left" as const, visible: false },
          ...
          },
          {
            ...
            onClick: async () => {
              ...
              const rows = gastosFiltrados.map((gasto) =>
                keys.map((key) => getGridColumnValue(gasto, key))
              );
              ...
              XLSX.writeFile(workbook, `vacaciones_export_${new Date().toISOString().slice(0,10)}.xlsx`);
            },
          },
        ]}
      />
...
{modo === "nuevo" ? "Nuevo registro" : actualizacionBloqueada || modo === "ver" ? "Visualizar registro" : "Editar registro"}
                  </h2>
                  <p style={{ marginTop: 8, marginBottom: 0, color: "#6B7280", fontSize: 13 }}>
                    Complete la informaciÃ³n del registro.
                  </p>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 20 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 24, color: "#17143A" }}>Nuevo registro de vacaciones</h2>
                <p style={{ marginTop: 8, marginBottom: 0, color: "#6B7280", fontSize: 13 }}>
                  Complete los datos requeridos para registrar la solicitud de vacaciones.
                </p>
              </div>
              <button
                type="button"
                onClick={cerrarVacacionPanel}
                style={{
                  border: "none",
                  background: "#F3F4F6",
                  color: "#17143A",
            ...
            <div style={{ display: "grid", gap: 14 }}>
              <div>
                ...
                <div style={{ position: "relative", width: "100%" }}>
                  <input
                    type="text"
                    value={
                      empleadoVacacionSeleccionado?.label ||
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
                <div>
                  <label style={{ display: "block", marginBottom: 6, fontSize: 11, fontWeight: 700, color: "#374151" }}>Fecha inicio</label>
                  <input
                    type="date"
                    value={vacacionForm.fechaInicio}
                    onChange={(e) => actualizarVacacionCampo("fechaInicio", e.target.value)}
                    disabled={vacacionGuardando}
                    style={{
                      width: "100%",
                      height: 42,
                      borderRadius: 10,
                      border: `1px solid ${vacacionErrores.fechaInicio ? "#F87171" : "#D1D5DB"}`,
                      padding: "0 12px",
                      fontSize: 12,
                      boxSizing: "border-box",
                    }}
                  />
                  {vacacionErrores.fechaInicio && <div style={{ fontSize: 12, color: "#DC2626", marginTop: 4 }}>{vacacionErrores.fechaInicio}</div>}
                </div>
                <div>
                  <label style={{ display: "block", marginBottom: 6, fontSize: 11, fontWeight: 700, color: "#374151" }}>Fecha fin</label>
                  <input
                    type="date"
                    value={vacacionForm.fechaFin}
                    onChange={(e) => actualizarVacacionCampo("fechaFin", e.target.value)}
                    disabled={vacacionGuardando}
                    style={{
                      width: "100%",
                      height: 42,
                      borderRadius: 10,
                      border: `1px solid ${vacacionErrores.fechaFin ? "#F87171" : "#D1D5DB"}`,
                      padding: "0 12px",
                      fontSize: 12,
                      boxSizing: "border-box",
                    }}
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 8 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    fontWeight: 600,
                    cursor: vacacionGuardando ? "not-allowed" : "pointer",
                  }}
                >
                  Cancelar
                </button>
                <button
                  type="button"
                  onClick={vacacionGuardando ? undefined : guardarVacacionNueva}
                  disabled={vacacionGuardando}
                  style={{
                    border: "none",
                    background: vacacionGuardando ? "#CBD5E1" : "#6E4CCB",
                    color: "#FFFFFF",
                    padding: "10px 16px",
                    borderRadius: 10,
                    fontWeight: 700,
                ...
                >
                  {vacacionGuardando ? "Guardando..." : "Guardar"}
                </button>
              </div>
            </div>
          </div>
