    ...
    async (status: number, start: string, end: string, correlativo?: number) => {
      ...
      try {
        const result = status === 100
          ? await listarPagosTesoreria(100, start, end, controller.signal, correlativo, bancosPagoFiltro.map(Number))
          : status === 99
      ...
      <div className="pt-page">
        ...
        {estado === 100 && (
          <form className="pt-filters pt-search-only" onSubmit={(e) => {
            ...
            const hasFilter = (Number.isInteger(parsed) && parsed > 0) || cliente || monedasFiltro.length || comprobantesFiltro.length || bancosCtaFiltro.length || bancosPagoFiltro.length || responsablesFiltro.length || solicitantesFiltro.length;
            if (!hasFilter) {
            ...
            <footer className="pt-table-footer">
              ...
              {requierePaginacion && (
                <div className="pt-pagination" aria-label="Paginación de registros">
                  ...
                  <button type="button" onClick={() => setCurrentPage((page) => Math.min(pageCount, page + 1))} disabled={saving || currentPage >= pageCount}>Siguiente</button>
                </div>
              )}
