...
function getLookupStoredValue(option: CrudLookupItem): string {
  return option.codigo || option.value || option.label;
}

function getLookupIdValue(option: CrudLookupItem): string {
  return option.value || option.codigo || option.label;
}

function getLookupOptionKey(option: CrudLookupItem): string {
  return getLookupIdValue(option);
}

function matchesLookupNumericValue(option: CrudLookupItem, normalized: string): boolean {
  const rawNumber = Number(normalized);
  if (!Number.isFinite(rawNumber)) {
    return false;
  }

  return [option.codigo, option.value, option.label].some((candidate) => {
    const parsed = Number(candidate);
    return Number.isFinite(parsed) && parsed === rawNumber;
  });
}

function resolveLookupSelectValue(options: CrudLookupItem[], rawValue: string | number | null | undefined): string {
  ...
  const byValue = options.find(
    (item) => getLookupStoredValue(item) === normalized || matchesLookupNumericValue(item, normalized)
  );
  if (byValue) {
    return getLookupIdValue(byValue);
  }

  const normalizedUpper = normalized.toUpperCase();
  const byLabel = options.find((item) => item.label.trim().toUpperCase() === normalizedUpper);
  if (byLabel) {
    return getLookupIdValue(byLabel);
  }
...
function mapItemToForm(
  ...
  const tipoDocumentoValue =
    item.idDocumento !== null && item.idDocumento !== undefined && item.idDocumento > 0
      ? item.idDocumento
      : item.tipoDoc;
  ...
  return {
    ...
    sexo: resolveLookupSelectValue(lookups?.sexos ?? [], item.idSexo ?? item.sexo),
    idDocumento: resolveLookupSelectValue(lookups?.tiposDocumento ?? [], tipoDocumentoValue),
  ...
  const openEdit = async (item: ExternoCrudItem) => {
    ...
    try {
      ...
      const source = {
        ...item,
        ...detalle,
        idSexo: detalle.idSexo ?? item.idSexo,
        idDocumento: detalle.idDocumento ?? item.idDocumento,
        sexo: normalizeTextValue(detalle.sexo) || item.sexo,
        tipoDoc: normalizeTextValue(detalle.tipoDoc) || item.tipoDoc,
      };
      setForm(
        mapItemToForm(
          source,
          currentLookups ? { sexos: currentLookups.sexos, tiposDocumento: currentLookups.tiposDocumento } : { sexos, tiposDocumento }
        )
      );
      setPanelOpen(true);
    } catch (err) {
            ...
            input={
              ...
              >
                ...
                {tiposDocumento.map((option) => (
                  <option key={`tipodoc-${getLookupOptionKey(option)}`} value={getLookupIdValue(option)}>
                    {option.label}
                  </option>
                ))}
              </select>
          ...
          <Field
            ...
            input={
              ...
              >
                <option value="">Seleccione...</option>
                {sexos.map((option) => (
                  <option key={`sexo-${getLookupOptionKey(option)}`} value={getLookupIdValue(option)}>
                    {option.label}
                  </option>
                ))}
              </select>
