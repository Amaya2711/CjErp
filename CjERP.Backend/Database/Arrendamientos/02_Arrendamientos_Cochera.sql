-- ROLLBACK-MARKER: ARRRENDAMIENTOS COCHERA START
-- Este script agrega cochera como cargo periodico independiente en contratos.

IF COL_LENGTH('dbo.a_contrato', 'MonedaCochera') IS NULL
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD MonedaCochera CHAR(3) NULL;
END;
GO

IF COL_LENGTH('dbo.a_contrato', 'MonedaGarantia') IS NULL
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD MonedaGarantia CHAR(3) NULL;
END;
GO

IF COL_LENGTH('dbo.a_contrato', 'ImporteCochera') IS NULL
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD ImporteCochera DECIMAL(18,2) NULL;
END;
GO

IF COL_LENGTH('dbo.a_contrato', 'PeriodicidadCochera') IS NULL
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD PeriodicidadCochera NVARCHAR(30) NULL;
END;
GO

IF COL_LENGTH('dbo.a_contrato', 'DiaLimiteCochera') IS NULL
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD DiaLimiteCochera INT NULL;
END;
GO

UPDATE dbo.a_contrato
SET MonedaCochera = ISNULL(NULLIF(LTRIM(RTRIM(MonedaCochera)), ''), ISNULL(NULLIF(LTRIM(RTRIM(MonedaAlquiler)), ''), ISNULL(NULLIF(LTRIM(RTRIM(Moneda)), ''), 'PEN'))),
    MonedaGarantia = ISNULL(NULLIF(LTRIM(RTRIM(MonedaGarantia)), ''), ISNULL(NULLIF(LTRIM(RTRIM(Moneda)), ''), 'PEN')),
    ImporteCochera = ISNULL(ImporteCochera, 0),
    PeriodicidadCochera = ISNULL(NULLIF(LTRIM(RTRIM(PeriodicidadCochera)), ''), 'MENSUAL'),
    DiaLimiteCochera = ISNULL(DiaLimiteCochera, 5)
WHERE MonedaCochera IS NULL
   OR MonedaGarantia IS NULL
   OR ImporteCochera IS NULL
   OR PeriodicidadCochera IS NULL
   OR DiaLimiteCochera IS NULL;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.a_contrato') AND name = 'MonedaCochera' AND is_nullable = 1)
BEGIN
    ALTER TABLE dbo.a_contrato
        ALTER COLUMN MonedaCochera CHAR(3) NOT NULL;
END;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.a_contrato') AND name = 'MonedaGarantia' AND is_nullable = 1)
BEGIN
    ALTER TABLE dbo.a_contrato
        ALTER COLUMN MonedaGarantia CHAR(3) NOT NULL;
END;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.a_contrato') AND name = 'ImporteCochera' AND is_nullable = 1)
BEGIN
    ALTER TABLE dbo.a_contrato
        ALTER COLUMN ImporteCochera DECIMAL(18,2) NOT NULL;
END;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.a_contrato') AND name = 'PeriodicidadCochera' AND is_nullable = 1)
BEGIN
    ALTER TABLE dbo.a_contrato
        ALTER COLUMN PeriodicidadCochera NVARCHAR(30) NOT NULL;
END;
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.a_contrato') AND name = 'DiaLimiteCochera' AND is_nullable = 1)
BEGIN
    ALTER TABLE dbo.a_contrato
        ALTER COLUMN DiaLimiteCochera INT NOT NULL;
END;
GO
...
IF NOT EXISTS (
    ...
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'DF_a_contrato_MonedaCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT DF_a_contrato_MonedaCochera DEFAULT ('PEN') FOR MonedaCochera;
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'DF_a_contrato_ImporteCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT DF_a_contrato_ImporteCochera DEFAULT ((0)) FOR ImporteCochera;
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'DF_a_contrato_PeriodicidadCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT DF_a_contrato_PeriodicidadCochera DEFAULT ('MENSUAL') FOR PeriodicidadCochera;
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.default_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'DF_a_contrato_DiaLimiteCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT DF_a_contrato_DiaLimiteCochera DEFAULT ((5)) FOR DiaLimiteCochera;
END;
GO
...
IF NOT EXISTS (
    ...
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'CK_a_contrato_MonedaCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT CK_a_contrato_MonedaCochera CHECK (LEN(MonedaCochera) = 3);
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.check_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'CK_a_contrato_MonedaGarantia'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT CK_a_contrato_MonedaGarantia CHECK (LEN(MonedaGarantia) = 3);
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.check_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'CK_a_contrato_ImporteCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT CK_a_contrato_ImporteCochera CHECK (ImporteCochera >= 0);
END;
GO
...
IF NOT EXISTS (
    ...
    FROM sys.check_constraints
    WHERE parent_object_id = OBJECT_ID('dbo.a_contrato')
      AND name = 'CK_a_contrato_DiaLimiteCochera'
)
BEGIN
    ALTER TABLE dbo.a_contrato
        ADD CONSTRAINT CK_a_contrato_DiaLimiteCochera CHECK (DiaLimiteCochera BETWEEN 1 AND 31);
END;
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM dbo.a_concepto
    WHERE CodigoConcepto = 'COCHERA'
)
BEGIN
    INSERT INTO dbo.a_concepto
    (
        CodigoConcepto,
        NombreConcepto,
        TipoConcepto,
...
CREATE OR ALTER PROCEDURE dbo.sp_a_Contrato_Guardar
    ...
    @MonedaCochera CHAR(3) = NULL,
    @MonedaGarantia CHAR(3) = NULL,
    ...
    @ImporteCochera DECIMAL(18,2) = 0,
    @PeriodicidadCochera NVARCHAR(30) = 'MENSUAL',
    @DiaLimiteCochera INT = 5,
...
BEGIN
    ...
    DECLARE @MonedaCocheraContrato CHAR(3) = ISNULL(NULLIF(LTRIM(RTRIM(@MonedaCochera)), ''), @MonedaContrato);
    DECLARE @MonedaGarantiaContrato CHAR(3) = ISNULL(NULLIF(LTRIM(RTRIM(@MonedaGarantia)), ''), @MonedaContrato);
    ...
    IF LEN(ISNULL(@MonedaContrato, '')) <> 3
       ...
       OR LEN(ISNULL(@MonedaCocheraContrato, '')) <> 3
       OR LEN(ISNULL(@MonedaGarantiaContrato, '')) <> 3
    BEGIN
    ...
    BEGIN
        ...
        (
            ...
            MonedaAlquiler, MonedaMantenimiento, MonedaCochera, MonedaGarantia, ImporteAlquiler, PeriodicidadAlquiler, DiaLimitePago, DiasGracia,
            ImporteMantenimiento, PeriodicidadMantenimiento, DiaLimiteMantenimiento, ImporteCochera, PeriodicidadCochera, DiaLimiteCochera,
        ...
        (
            ...
            @MonedaAlquilerContrato, @MonedaMantenimientoContrato, @MonedaCocheraContrato, @MonedaGarantiaContrato, ISNULL(@ImporteAlquiler, 0), ISNULL(@PeriodicidadAlquiler, 'MENSUAL'), ISNULL(@DiaLimitePago, 5), ISNULL(@DiasGracia, 0),
            ISNULL(@ImporteMantenimiento, 0), ISNULL(@PeriodicidadMantenimiento, 'MENSUAL'), ISNULL(@DiaLimiteMantenimiento, 5), ISNULL(@ImporteCochera, 0), ISNULL(@PeriodicidadCochera, 'MENSUAL'), ISNULL(@DiaLimiteCochera, 5),
        ...
        (
            SELECT
                ...
                @MonedaCocheraContrato AS MonedaCochera,
                @MonedaGarantiaContrato AS MonedaGarantia,
                @ImporteAlquiler AS ImporteAlquiler,
                @ImporteMantenimiento AS ImporteMantenimiento,
                @ImporteCochera AS ImporteCochera,
    ...
    BEGIN
        ...
        SET CodigoContrato = LTRIM(RTRIM(@CodigoContrato)),
            ...
            MonedaCochera = @MonedaCocheraContrato,
            MonedaGarantia = @MonedaGarantiaContrato,
            ...
            ImporteCochera = ISNULL(@ImporteCochera, 0),
            PeriodicidadCochera = ISNULL(@PeriodicidadCochera, 'MENSUAL'),
            DiaLimiteCochera = ISNULL(@DiaLimiteCochera, 5),
        ...
        VALUES
        (
            @IdContrato, 'MODIFICACION', SYSDATETIME(), @Usuario, 'Actualizacion del contrato.', @AnteriorJson, @NuevoJson,
            @DocumentoFirmadoNombre, @DocumentoFirmadoUrl, @DocumentoFirmadoTamanoKB, @Usuario, SYSDATETIME()
        );
...
-- ROLLBACK-MARKER: ARRRENDAMIENTOS COCHERA END
